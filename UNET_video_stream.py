...
from keras.layers import Input, Conv2D, MaxPool2D, UpSampling2D, BatchNormalization, Concatenate
...
def unet():
    inputs = Input((heights, width, 3))
    x = Conv2D(8, 3, 3, activation='relu', padding='same')(inputs)
    x= BatchNormalization()(x)
    x = Conv2D(8, 3, 3, activation='relu', padding='same')(x)
    downsample1 = BatchNormalization()(x)

    x = MaxPool2D(pool_size=(2, 2))(downsample1)
    x = Conv2D(16, 3, 3, activation='relu', padding='same')(x)
    x = BatchNormalization()(x)
    x = Conv2D(16, 3, 3, activation='relu', padding='same')(x)
    downsample2 = BatchNormalization()(x)

    x = MaxPool2D(pool_size=(2, 2))(downsample2)
    x = Conv2D(32, 3, 3, activation='relu', padding='same')(x)
    x = BatchNormalization()(x)
    x = Conv2D(32, 3, 3, activation='relu', padding='same')(x)
    downsample3 = BatchNormalization()(x)

    x = MaxPool2D(pool_size=(2, 2))(downsample3)
    x = Conv2D(64, 3, 3, activation='relu', padding='same')(x)
    x = BatchNormalization()(x)
    x = Conv2D(64, 3, 3, activation='relu', padding='same')(x)
    downsample4 = BatchNormalization()(x)

    x = MaxPool2D(pool_size=(2, 2))(downsample4)
    x = Conv2D(128, 3, 3, activation='relu', padding='same')(x)
    x = BatchNormalization()(x)
    x = Conv2D(128, 3, 3, activation='relu', padding='same')(x)
    upsample1 = BatchNormalization()(x)

    x = Concatenate(axis=3)(UpSampling2D(size=(2, 2))(upsample1), downsample4)
    x = Conv2D(64, 3, 3, activation='relu', padding='same')(x)
    x = BatchNormalization()(x)
    x = Conv2D(64, 3, 3, activation='relu', padding='same')(x)
    upsample2 = BatchNormalization()(x)

    x = Concatenate(axis=3)(UpSampling2D(size=(2, 2))(upsample2), downsample3)
    x = Conv2D(32, 3, 3, activation='relu', padding='same')(x)
    x = BatchNormalization()(x)
    x = Conv2D(32, 3, 3, activation='relu', padding='same')(x)
    upsample3 = BatchNormalization()(x)

    x = Concatenate(axis=3)(UpSampling2D(size=(2, 2))(upsample3), upsample1)
    x = Conv2D(16, 3, 3, activation='relu', padding='same')(x)
    x = BatchNormalization()(x)
    x = Conv2D(16, 3, 3, activation='relu', padding='same')(x)
    upsample4 = BatchNormalization()(x)

    x = Concatenate(axis=3)(UpSampling2D(size=(2, 2))(upsample4), downsample1)
    x = Conv2D(8, 3, 3, activation='relu', padding='same')(x)
    x = BatchNormalization()(x)
...
def boundary_labeler(img, labels):
...
def predictor(img):
    ...
    img = cv2.resize(img, (width, heights))
    img = np.reshape(img, (1, heights, width, 3))
    pred = model.predict(img)
    return pred, img[0]


# def get_boundary_pred(img):
#     # Get bounding boxes
#     pred, img = predictor(img)
#     img = np.array(img, dtype=np.uint8)
#     img_pred = np.array(255 * pred[0], dtype=np.uint8)
#     heatmap = img_pred[:, :, 0]
#     heatmap = smoothing(heatmap)
#     labels = label(heatmap)
#     draw_img = boundary_labeler(np.copy(img), labels)
#     return draw_img


def get_boundary_label_pred(img, labels):
    # Get labeled boxex
    bbox_all = []
    for car_number in range(1, labels[1] + 1):
        # Find pixels with each car_number label value
...
def get_boundary_new(img):
    ...
    bbox_all = get_boundary_label_pred(np.copy(img), labels)
    return bbox_all
...
def pipeline_process_highway(image, do_diagnosis):
    ...
    bbox_cars = get_boundary_new(image_bb)
    ...
    if do_diagnosis == 1:
        ...
        diagScreen[840:1080, 320:640] = cv2.resize(stack_arr(255 * image_cmb1), (320, 240),
                                                   interpolation=cv2.INTER_AREA)
...
white_clip.write_videofile(challenge_output_diag, audio=False);
